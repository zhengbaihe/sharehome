...
export function HouseholdDetail({ token, householdId }: { token: string; householdId: string }) {
  ...
  return <section>
    ...
    {loading ? <p role="status">Loading household…</p> : error ? <p role="alert">{error}</p> : household &&
      <article className="auth-card"><h2>{household.name}</h2><HouseholdInfo household={household} /><Link to={`/households/${household.id}/bills`}>Bills</Link>{' · '}<Link to={`/households/${household.id}/balances`}>Balances</Link></article>}
  </section>;
}
